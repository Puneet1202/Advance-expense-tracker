...
// ── REFACTORED: aiChatHandler inside tracker.controller.js ──────────────────
export const aiChatHandler = async (c) => {
    try {
        ...
        // Call the decoupled core AI engine
        const result = await aiChat(c.env, supabase, user.id, message, history);

        // ======================================================================
        // 🚀 HTTP LAYER ACTION EXECUTION
        // ======================================================================
        if (result.action) {
            ...
            if (action === "ADD_TRANSACTION") {
                if (!data.amount || !data.type || !data.account_id) {
                    return c.json({ reply: "Transaction details are incomplete. Please specify both the amount and account." }, 200);
                }
                ...
                return c.json({ 
                   ...
                   reply: `Successfully added the transaction "${data.description}" worth ₹${data.amount} under ${finalCategory}.`
                }, 200);
            }
            ...
            if (action === "DELETE_TRANSACTION") {
                ...
                return c.json({ 
                    ...
                    reply: `Successfully deleted the transaction "${matchTxn.description}" worth ₹${matchTxn.amount}.` 
                }, 200);
            }
            ...
            if (action === "UNDO_LAST_ACTION") {
                ...
                if (lastTxn) {
                    ...
                    return c.json({ 
                        ...
                        reply: `Undo action completed. Removed the last transaction: "${lastTxn.description}" worth ₹${lastTxn.amount}.` 
                    }, 200);
                }
                return c.json({ reply: 'No recent transactions found to undo.' }, 200);
            }
        }
        ...
        return c.json({ message: 'Internal server error occurred while processing AI chat.', status: 500 }, 500);
    }
};
