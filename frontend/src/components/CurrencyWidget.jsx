    ...
    <div className="card anim-up d3" style={{ padding:'1.25rem' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          ...
          <p style={{ fontWeight:700, fontSize:'0.875rem', letterSpacing:'-0.02em', color:'var(--text-1)' }}>
            Live Rates
          </p>
        </div>
        <button
          ...
          style={{
            ...
            fontSize:'0.72rem', fontWeight:500, color:'var(--accent)',
            opacity: loading ? 0.4 : 1, fontFamily:'inherit'
          }}
      ...
      {!loading && !error && rates && (
        <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
          {/* Exchange Rate */}
          <div style={{
            background:'var(--accent-glow)', borderRadius:'12px', padding:'14px',
            border:'1px solid rgba(91,91,214,0.22)'
          }}>
            <p style={{ fontSize:'0.68rem', fontWeight:600, color:'var(--text-3)',
              letterSpacing:'0.04em', textTransform:'uppercase', marginBottom:'6px' }}>
              EXCHANGE RATE
            </p>
            <div className="num" style={{ display:'flex', alignItems:'baseline', gap:'6px', flexWrap:'wrap' }}>
            ...
            <p style={{ fontSize:'0.68rem', color:'var(--text-4)', marginTop:'4px' }}>
          ...
          {/* Purchasing Power */}
          <div style={{
            background:'var(--green-bg)', borderRadius:'12px', padding:'14px',
          ...
          {/* Last Updated */}
          {lastUpdated && (
            <p style={{ fontSize:'0.68rem', textAlign:'center', color:'var(--text-4)', marginTop:'2px' }}>
              🕐 {lastUpdated.toLocaleDateString()} {lastUpdated.toLocaleTimeString()}
              <span style={{ marginLeft:'4px', opacity:0.5 }}>• 12h cache</span>
            </p>
          )}
        </div>
      )}
