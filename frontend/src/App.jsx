...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    try { return localStorage.getItem('theme_guest') === 'dark'; } catch { return false; }
  });
  ...
  useEffect(() => {
    const timer = setTimeout(() => { fetchTrackerData(); }, 400);
  ...
  if (isLoading) return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.25rem', background: 'var(--bg)' }}>
      <div style={{ width: '52px', height: '52px', borderRadius: '16px', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', boxShadow: '0 8px 28px var(--accent-glow)' }}>💳</div>
      <div style={{ display: 'flex', gap: '5px' }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ width: '7px', height: '7px', borderRadius: '50%', background: 'var(--accent)', animation: `pulse 1.2s ease-in-out ${i * 0.18}s infinite` }} />
        ))}
      </div>
  ...
  if (!isLoggedIn) return (
    <AuthForm onLoginSuccess={fetchTrackerData} />
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: darkMode ? '#0f172a' : '#f8fafc', transition: 'background 0.3s' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: isMobile ? '0.6rem 0.6rem 3rem' : '1.25rem 1.25rem 3rem', boxSizing: 'border-box', width: '100%' }}>
        ...
        {/* Filter Section */}
        <div style={{
          background: darkMode ? '#1e293b' : '#ffffff', padding: isMobile ? '14px' : '20px',
          borderRadius: '16px', marginBottom: '16px',
          border: `1px solid ${darkMode ? '#334155' : '#e2e8f0'}`,
          borderLeft: '4px solid #3b82f6', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)',
        }}>
          <div onClick={() => isMobile && setFilterOpen(o => !o)}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: filterOpen ? '16px' : 0, cursor: isMobile ? 'pointer' : 'default' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '1.1rem' }}>🔍</span>
              <h2 style={{ fontSize: isMobile ? '14px' : '16px', fontWeight: 'bold', color: darkMode ? '#f8fafc' : '#0f172a', margin: 0 }}>Filter Your Dashboard</h2>
            </div>
            {isMobile && <span style={{ fontSize: '0.75rem', color: 'var(--text-4)', transform: filterOpen ? 'rotate(180deg)' : 'none', display: 'inline-block' }}>▼</span>}
          </div>

          {filterOpen && (
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? '1fr 1fr' : '2fr 1fr 1.5fr', gap: isMobile ? '10px' : '16px' }}>
              <div>
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Salary, Rent, HDFC..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={inputStyle} />
              </div>
            </div>
          )}
        </div>
        ...
        {isMobile ? (
          <div style={{ marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', borderRadius: '14px', padding: '14px 16px', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <p style={{ fontSize: '10px', opacity: 0.75, fontWeight: 600, marginBottom: '2px' }}>TOTAL BALANCE</p>
                <h1 style={{ fontSize: '22px', fontWeight: 800, margin: 0 }}>₹{totalBalance.toLocaleString('en-IN')}</h1>
              </div>
              <p style={{ fontSize: '11px', opacity: 0.65 }}>{(trackerData.accounts || []).length} accounts</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
              {[
                ...
                <div key={c.label} style={{ background: c.bg, borderRadius: '12px', padding: '10px', border: `1px solid ${c.border}` }}>
                  ...
                  <p style={{ fontSize: '13px', fontWeight: 800, color: c.color, margin: 0 }}>{c.prefix || ''}₹{c.value.toLocaleString('en-IN')}</p>
                </div>
              ))}
            </div>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
            <div style={{ background: 'linear-gradient(135deg,#8b5cf6,#6366f1)', borderRadius: '16px', padding: '20px', color: '#fff' }}>
